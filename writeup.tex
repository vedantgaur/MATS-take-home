...
Figure~\ref{fig:within_process} shows the results. Where triangular structure has crystallized, three distinct clusters should appear corresponding to the three vertices of the Mess3 2-simplex. The degree to which this structure has or has not formed provides a direct window into the microscopic state-tracking question: the high $R^2$ tells us the model encodes belief-relevant information, but only the geometry reveals whether it has organized that information into the simplex structure predicted by the theory.
